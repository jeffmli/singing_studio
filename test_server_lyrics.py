"""Tests for synced-lyric surfacing in server.py (no network).
...
def test_extract_lrclib_item():
    plain, synced = server._extract_lrclib(
...
def test_fetch_lyrics_synced_empty_when_only_ovh(monkeypatch):
    monkeypatch(server, "_lrclib_get", lambda a, s: {"plain": "", "synced": ""})
    monkeypatch(server, "_lrclib_search", lambda q: {"plain": "", "synced": ""})
    monkeypatch(server, "_lyrics_ovh", lambda a, s: "from ovh\nsecond line")
    plain, synced = server.fetch_lyrics("X", "Y", "x y")
